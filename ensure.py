...
def folder_names(root, lfmt):
    for img_fol, lab_fol in zip(sorted(os.listdir(
            root + "/images")), sorted(os.listdir(root + "/labels{}".format(lfmt)))):
...
def file_names(root, lfmt):
    ...
    for img_fol, lab_fol in zip(sorted(os.listdir(
        ...
        for img_file, lab_file in zip(
            sorted(
                os.listdir(
                    root + "/images/" + img_fol)), sorted(
                os.listdir(
                    root + "/labels{}/".format(lfmt) + lab_fol))):
            if img_file[:-4] != lab_file[:-4]:
                return img_fol + "/" + img_file + " and " + \
                    lab_fol + "/" + lab_file + " doesn't match!"
...
def store(root, lfmt):
    img_names = {}
    lab_names = {}
    for img_fol, lab_fol in zip(os.listdir(root +
        ...
        for img_file, lab_file in zip(
            ...
            if img_file not in img_names:
                ...
                return img_names[img_file] + "/" + img_file + " and " + \
                    img_fol + "/" + img_file + " matches! (SAME FILE NAME)!"

            if lab_file not in lab_names:
                lab_names[lab_file] = lab_fol
            else:
                return lab_names[lab_file] + "/" + lab_file + " and " + \
                    lab_fol + "/" + lab_file + " matches! (SAME FILE NAME)!"
    return "OK"
